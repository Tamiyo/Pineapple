...
pub fn dead_code_elimination(context: &mut ControlFlowContext) {
    ...
    let mut usages: HashMap<Operand, Vec<(Rc<RefCell<Stmt>>, usize)>> = HashMap::new();
    let mut definitions: HashMap<Operand, (Rc<RefCell<Stmt>>, usize)> = HashMap::new();

    for (i, b) in context.cfg.blocks.iter().enumerate() {
        for s in b.statements.iter() {
            for v in &s.borrow().vars_defined() {
                w.insert(*v);
                definitions.insert(*v, (s.clone(), i));
            }
            for v in &s.borrow().vars_used() {
                if !w.contains(&*v) {
                    w.insert(*v);
                }
...
fn remove_statement_and_patch(
    ...
    w: &mut Vec<Operand>,
    definitions: &mut HashMap<Operand, (Rc<RefCell<Stmt>>, usize)>,
    usages: &mut HashMap<Operand, Vec<(Rc<RefCell<Stmt>>, usize)>>,
) {
    ...
    context.cfg.blocks[*b].remove_statement(Rc::clone(s));
}
